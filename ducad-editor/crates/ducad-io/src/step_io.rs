...
//! JSON `.ducad` — modul ini cuma tipis di atas `ducad-kernel`, isinya
...
pub fn export(shapes: &[&KernelShape], path: impl AsRef<Path>) -> Result<()> {
    match shapes {
        ...
        many => ducad_kernel::write_step_compound(many, path),
    }
}
...
mod tests {
    ...
    fn export_single_then_import_preserves_mesh() {
        ...
        let path = std::env::temp_dir().join(format!("ducad-io-test-step-single-{}.step", std::process::id()));
    ...
    fn export_many_combines_bodies() {
        ...
        let path = std::env::temp_dir().join(format!("ducad-io-test-step-many-{}.step", std::process::id()));
    ...
    fn export_empty_errors() {
        let path = std::env::temp_dir().join("ducad-io-test-step-empty.step");
