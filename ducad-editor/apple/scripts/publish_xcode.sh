#!/bin/bash
# ==============================================================================
# DUCAD Xcode Archive & App Store Connect Publisher (CLI Helper)
# ==============================================================================
# Usage:
...
ROOT_DIR="$(dirname "$EDITOR_DIR")"

# Load .env credentials if available
if [ -f "$ROOT_DIR/.env" ]; then
    set -a; source "$ROOT_DIR/.env"; set +a
elif [ -f "$EDITOR_DIR/.env" ]; then
    set -a; source "$EDITOR_DIR/.env"; set +a
fi

cd "$EDITOR_DIR"

PROJECT_FILE="DUCAD.xcodeproj"
DIST_DIR="$EDITOR_DIR/dist/xcode"
mkdir -p "$DIST_DIR"
...
archive_ios() {
    echo "📱 [Xcode Build] Archiving DUCAD-iOS (iPadOS)..."
    local archive_path="$DIST_DIR/DUCAD-iOS.xcarchive"
    rm -rf "$archive_path"
...
archive_macos() {
    echo "💻 [Xcode Build] Archiving DUCAD-macOS..."
    local archive_path="$DIST_DIR/DUCAD-macOS.xcarchive"
    rm -rf "$archive_path"
