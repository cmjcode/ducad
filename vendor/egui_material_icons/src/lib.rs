...
pub(crate) const FONT_DATA_MDI: &[u8] =
    include_bytes!("../materialdesignicons-webfont.ttf");
...
/// The font family name used for Pictogrammers Material Design Icons (MDI).
pub const FONT_FAMILY_MDI: &str = "materialdesignicons";
...
/// Creates a [`FontInsert`] for the Pictogrammers Material Design Icons (MDI) font.
pub fn font_insert_mdi() -> FontInsert {
    let mut data = FontData::from_static(FONT_DATA_MDI);
    data.tweak.y_offset_factor = 0.05;
    ...
    FontInsert::new(
        ...
        vec![
            ...
            InsertFontFamily {
                family: FontFamily::Name(FONT_FAMILY_MDI.into()),
                priority: FontPriority::Highest,
            },
        ],
    )
    ...
    fn test_font_initialization() {
        ...
        let _ = ctx.run(Default::default(), |ctx| {
            egui::CentralPanel::default().show(ctx, |ui| {
                ui.label(icons::ICON_HOME.rich_text());
                ui.label(icons::ICON_SETTINGS.rich_text());
                ui.label(icons::ICON_EDIT.rich_text());
                ui.label(icons::ICON_LAYERS.rich_text());
            });
        });
    }
}
