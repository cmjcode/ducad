...
pub mod ffi {
    ...
    unsafe extern "C++" {
        ...
        // Result<> lewat wrapper try/catch di wrapper.hxx (lihat komentar
        // `rethrow_standard_failure_as_runtime_error`), supaya tertangkap
        // sebagai error Rust, bukan abort proses.
        type BRepAdaptor_Surface;
        type gp_Cylinder;
        type gp_Cone;
        // Alias ke `enum GeomAbs_SurfaceType` OCCT asli (bukan `enum class`
        // baru bikinan cxx) — wrapper.hxx sudah `#include
        // <GeomAbs_SurfaceType.hxx>`, jadi tanpa deklarasi `type` di sini
        // cxx akan coba definisikan ULANG `enum class GeomAbs_SurfaceType`
        // dan bentrok ("enumeration previously declared as unscoped") —
        // pola yang sama dipakai `TopAbs_ShapeEnum`/`BOPAlgo_GlueEnum` di
        // atas.
        ...
        pub fn BRepAdaptor_Surface_cone(surface: &BRepAdaptor_Surface) -> Result<UniquePtr<gp_Cone>>;
        ...
        // melempar Standard_Failure lewat sisi C++) dideklarasikan
        // Result<> — diterjemahkan lewat wrapper try/catch di wrapper.hxx,
        // BUKAN mekanisme auto-catch cxx (yang cuma nangkep std::exception,
        // sedang Standard_Failure turunan Standard_Transient) — lihat
        // komentar `rethrow_standard_failure_as_runtime_error`.
        type BRepOffset_MakeOffset;
        // Alias ke enum OCCT asli, sama alasannya dengan GeomAbs_SurfaceType
        // di atas (wrapper.hxx sudah `#include <BRepOffset_Mode.hxx>` /
