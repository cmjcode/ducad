...
use cadraw_sketch::{Entity, Sketch};
use glam::DVec2;
use std::path::Path;
...
mod tests {
    ...
    fn export_reports_one_skipped_ellipse() {
        ...
        let path = std::env::temp_dir().join(format!("cadraw-io-test-{}.dxf", std::process::id()));
    ...
    fn export_then_import_roundtrips_line_circle_arc() {
        let sketch = sample_sketch();
        let path = std::env::temp_dir().join(format!("cadraw-io-test-roundtrip-{}.dxf", std::process::id()));
    ...
    fn import_missing_entities_section_returns_empty() {
        let path = std::env::temp_dir().join(format!("cadraw-io-test-nosec-{}.dxf", std::process::id()));
    ...
    fn import_skips_unsupported_entity_types() {
        let dxf = "0\nSECTION\n2\nENTITIES\n0\nTEXT\n1\nhello\n0\nLINE\n8\n0\n10\n0.0\n20\n0.0\n30\n0.0\n11\n1.0\n21\n1.0\n31\n0.0\n0\nENDSEC\n0\nEOF\n";
        let path = std::env::temp_dir().join(format!("cadraw-io-test-skip-{}.dxf", std::process::id()));
