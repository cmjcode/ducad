...
impl CadrawApp {
    ...
    pub fn save_native_to(&mut self, path: PathBuf) {
        ...
        match cadraw_io::native::save_multi_plane(&path, &self.sketches, &body_refs) {
            Ok(_) => {
                let name = path
                    .file_name()
                    .and_then(|n| n.to_str())
    ...
    pub fn save_native_as(&mut self) {
        if let Some(path) = self.pick_save_path("Dokumen CADRAW", &["cadraw"], "model.cadraw") {
            self.save_native_to(path);
        }
    }

    pub fn open_native(&mut self) {
        let Some(path) = self.pick_open_path("Dokumen CADRAW", &["cadraw"]) else {
            return;
        };
    ...
    pub fn export_stl(&mut self) {
        ...
        let mesh_refs: Vec<&cadraw_kernel::KernelMesh> =
            meshes.iter().map(|(_, m)| *m).collect();
        let merged = cadraw_kernel::KernelMesh::merge(&mesh_refs);
    ...
    pub fn export_obj(&mut self) {
        ...
        match cadraw_io::mesh_export::write_obj(&meshes, &path) {
            Ok(_) => {
                let name = path
                    .file_name()
                    .and_then(|n| n.to_str())
    ...
    pub fn import_dxf(&mut self) {
        ...
        match cadraw_io::dxf::import(&path) {
            Ok(res) => {
                ...
                self.execute_sketch_command(Box::new(cadraw_sketch::InsertEntities::new(
                    "Impor DXF",
                    res.entities,
                )));
                let name = path
